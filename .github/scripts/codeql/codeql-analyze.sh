...
then 
    ...
    < java.sarif jq 'del(.runs[].results[].codeFlows)' > "$GITHUB_WORKSPACE"/results/processed-java.sarif
...
then
    ...
    < themes.sarif jq 'del(.runs[].results[].codeFlows)' > "$GITHUB_WORKSPACE"/results/processed-themes.sarif
    ...
    < js-adapter.sarif jq 'del(.runs[].results[].codeFlows)' > "$GITHUB_WORKSPACE"/results/processed-js-adapter.sarif
